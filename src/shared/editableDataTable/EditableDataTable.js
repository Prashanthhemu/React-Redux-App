...
function EditableDataTable({ data, deleteRow }) {
    var columns = [
    ...
    const handleRowUpdate = (newData, oldData, resolve) => {
        ...
        if (errorList.length < 1) {
            api.put("/User/" + oldData.id, JSON.stringify(newData), { headers: { 'Content-Type': 'application/json' } })
                .then(response => {
    ...
    const handleRowAdd = (newData, resolve) => {
        ...
        if (errorList.length < 1) { //no error
            api.post("/User", JSON.stringify(newData), { headers: { 'Accept': 'application/json', 'Content-Type': 'application/json' } })
                .then(res => {
                    let dataToAdd = [...data];
